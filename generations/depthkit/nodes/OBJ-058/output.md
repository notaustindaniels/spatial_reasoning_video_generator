# Specification: OBJ-058 — Director Agent Workflow Specification
...
export interface TestRenderConfig {
   ...
   *
   * - 'draft': 640x360 (landscape base), crf 28, 24fps. Fastest. For initial iteration.
   * - 'review': 1280x720 (landscape base), crf 23, 30fps. For Director Agent review.
   * - 'proof': 1920x1080 (landscape base), crf 20, 30fps. Near-production. For final sign-off.
   *
...
Each tuning objective (OBJ-059 through OBJ-066, OBJ-068) tracks its rounds in a `tuning-log.json` file within its node directory. A Zod schema for `TuningLog` validation is provided alongside the TypeScript interfaces in `src/tools/tuning-log-schema.ts`. Tuning sessions validate the log on read and write.
...
- [ ] **AC-14:** Quality presets override the manifest's resolution and fps. For landscape manifests: `draft` -> 640x360@24fps, `review` -> 1280x720@30fps, `proof` -> 1920x1080@30fps. For portrait manifests: width and height are swapped (e.g., `review` -> 720x1280@30fps).
...
| Manifest has portrait orientation (1080x1920) with `quality: 'review'` | `resolveQualityDimensions()` detects aspect ratio < 1.0, swaps width/height -> 720x1280@30fps. |
...
| `quality: 'proof'` with a 1920x1080 landscape manifest | Resolution matches — no override needed. CRF and fps are still applied from the preset. |
...
2. **Quality preset application — landscape:** For a 1920x1080 manifest with `quality: 'draft'`, verify the cloned manifest has `width: 640, height: 360, fps: 24`.

3. **Quality preset application — portrait:** For a 1080x1920 manifest with `quality: 'review'`, verify `resolveQualityDimensions()` returns `{ width: 720, height: 1280, crf: 23, fps: 30 }`.
...
- **TC-15:** Tests 19, 20 measure convergence round count.
- **TC-16:** Tests 21, AC-28, AC-29 define and verify the paired comparison framework.
...
Current decision: aspect ratio >= 1.0 uses landscape base dimensions. For 1:1 manifests (e.g., 1080x1080), `resolveQualityDimensions()` returns the landscape preset (1280x720 for `review`). This doesn't preserve the 1:1 ratio. If 1:1 video becomes important, add a third branch: `min(preset.width, preset.height) x min(preset.width, preset.height)`. Deferred — no 1:1 requirement in the seed.
