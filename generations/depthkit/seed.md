...
**Codename:** `depthkit`  
**Expected Scale:** 80–150 objectives. The engine infrastructure (rendering pipeline, manifest validation, CLI, scene sequencer) accounts for roughly half; the scene geometries, camera presets, and their visual tuning cycles account for the other half. This estimate informs the initializer's decomposition granularity and the integrator's sampling cadence.  
...
These are specific assertions that exploration sessions should verify or refute. Each may become a node in the progress map (tracked in `index.json` with its own node directory).
...
Every scene geometry and its default camera path preset has been through at least one complete Director Agent → HITL → Code Agent tuning cycle, and has been signed off by the human as cinematically acceptable. Each geometry node's `meta.json` shows `"visual_status": "tuned"` (mirrored in `index.json`) before the engine is considered production-ready.
...
│     in its node's meta.json and in index.json.                  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
The Director Agent is invoked **only** for the following objective types in the DAG:
...
- Final visual sign-off on a completed geometry before marking it as `"verified"` in its node's `meta.json` and `index.json`.
...
4. The preset is marked as `"visual_status": "tuned"` in its node's `meta.json` and mirrored in `index.json`.
...
- [ ] I have read the current `index.json` (the lightweight progress map index).
- [ ] I know which objective I am working on and have read its `meta.json`.
- [ ] I have read the `output.md` of each node in my objective's `depends_on` list.
- [ ] I know the vocabulary and will use it consistently.
- [ ] I know the constraints and will respect them.
- [ ] I will commit a discrete, reviewable artifact to my node's directory before my context fills up.
- [ ] I will report my status change to the orchestrator so it can update `index.json`.
